...
constants listing. `manifest.yaml` indexes every record. Pass 2 transcribed the
111 records that pass 1 left as explicit OCR/layout gaps, using MinerU tables
cross-checked against rendered page images. A counted placeholder does
not mean a table was successfully transcribed. The manifest summary separates
transcribed and untranscribed totals.
...
- `records/`: 330 high-temperature substance-table slots, printed pp. 67–396.
  Printed p. 176 (DIBORON TRIOXIDE) is transcribed from the MinerU table and
  the rendered page image; the original pdftotext layer had a heading but no
  table body.
...
kept under `/private/tmp`. Pass 2 used MinerU tables plus rendered page images
for the 111 records pass 1 could not assign to columns. Independent
...
certified. Where MinerU and the page image disagree, the raw tokens are kept
and marked `ocr_suspect`.
...
numbers for batches. `pass2_mineru_transcribe.py` fills the 111 pass-1
OCR/layout gaps from MinerU tables and rendered page images; it does not
rewrite already-transcribed records. `build_manifest.py` uses the census and native records;
