...
**`VapoRockBackend` and `MAGEMinBackend` are explicitly refused as the active `MeltBackend`.** Their honest call sites are now per-intent kernel `ChemistryProvider` registrations (VapoRock diagnostic shadow for `VAPOR_PRESSURE`; MAGEMin shadow for `SILICATE_LIQUIDUS` / `SILICATE_EQUILIBRIUM` under `\goal MAGEMIN-SHADOW-PARITY`), not the active-backend `_get_equilibrium` path. Selecting either as the active `MeltBackend` would still fail closed inside `simulator/core.py::_get_equilibrium` because their populated `phase_masses_kg` (MAGEMin) or vapor-only (VapoRock) returns leave `EquilibriumResult.ledger_transition=None`, which trips the "backend returned post-equilibrium phase material without an AtomLedger transition" reject. `resolve_backend('vaporock', ...)` and `resolve_backend('magemin', ...)` raise `BackendUnavailableError` under both selection policies — `WEB_AUTODETECT` reports the explicit not-eligible reason, `RUNNER_STRICT` reports them as unknown backend names. The kernel itself was carved out in `\goal CHEMISTRY-KERNEL-CARVE-OUT` and is the canonical home for the builtin-authoritative / VapoRock-shadow `VAPOR_PRESSURE` split; see the "VapoRock diagnostic shadow" section below.

`resolve_backend` takes an explicit `BackendSelectionPolicy` — `WEB_AUTODETECT` (the web/API `auto` path, which probes) or `RUNNER_STRICT` (the deterministic CLI runner and the session-script harness, which refuses `auto` and requires a named backend).

There is **no silent cross-backend fallback at runtime**. If the selected primary throws inside `_get_equilibrium` after selection, the existing fail-closed path in `simulator/core.py` handles it; `resolve_backend` does not re-probe a different primary mid-run.

On the `WEB_AUTODETECT` path, `simulator/backends.py::emit_web_engine_selection_log` emits one log line of the form
`engine selection: <BackendClassName> (backend_status=..., authoritative=..., capabilities: silicate_melt=..., gas_volatiles=...) -- VapoRock/MAGEMin not eligible until kernel`
so the active-backend choice is visible in the launcher's normal stdout stream. `RUNNER_STRICT` selection does not emit this line — the runner records the same information in `run_metadata` instead. The log line's wording predates the kernel-provider registrations; the message refers to active-`MeltBackend` eligibility (still gated), not to kernel `ChemistryProvider` registration (which VapoRock and MAGEMin now have via `engines/vaporock` and `engines/magemin`).
...
The other kernel-authoritative builtins are unchanged; their `authoritative` slot still names the `builtin-<intent>` provider. They are the fifteen rows of `PyrolysisSimulator._BUILTIN_PROVIDER_REGISTRATIONS` — `EVAPORATION_FLUX`, `EVAPORATION_TRANSITION`, `CONDENSATION_ROUTE`, `ELECTROLYSIS_STEP`, `METALLOTHERMIC_STEP`, `CA_ALUMINOTHERMIC_STEP`, `NATIVE_FE_SATURATION`, `NATIVE_FE_METALLIC_TAP`, `FE_REDOX_RESPECIATION`, `STAGE0_PRETREATMENT`, `OVERHEAD_GAS_EQUILIBRIUM`, `OVERHEAD_BLEED`, `OXYGEN_BUBBLER`, `OXYGEN_RESERVOIR_EXCHANGE`, `BACKEND_EQUILIBRIUM` — enumerated with their declared accounts in `docs/developer-map.md` under "Engine Source Trees".
...
- When either backend is selected, its diagnostic provider intentionally owns the narrow `GATE_LIQUID_FRACTION` dispatch because it is the selected backend's liquid-fraction source; this is dispatch-only authority. Runner provenance emits `run_metadata.diagnostic_gate_authority_notice`, copies it into the operator product-classification markdown and P11 flag, and forwards it through optimizer traces and product summaries. The MAGEMin provider has the same empty ledger-transition authority but is only a fallback, so it is not flagged.
