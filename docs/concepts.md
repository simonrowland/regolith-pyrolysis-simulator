...
1. **Metals + O₂** — the main extraction targets. Na and K condense in Stage 4 (alkali cyclone), Fe condenses in Stage 1 (Fe condenser), Mg in Stage 4, Al via Mg thermite in optional C6. Under the V1c JANAF refit, Na can clean residual FeO only in the cool ~1150 °C window; K is a volatile product/recyclable alkali stock, not a practical FeO reductant in the melt window. Disproportionation and reduction reactions release O₂, accumulated at the terminal as the by-product of extracting metal from oxide. The canonical recipe runs C0 → staged C2A → C3 → C4 Mg recovery → final continuous C2A boiloff. C5/MRE and C6 are excluded from that process-minimal profile.
...
- MgO more stable than Al₂O₃ per mol O₂ below the 1471.4 °C JANAF-4th multiphase crossover → Mg can take O from Al₂O₃ (C6 thermite). The 2026-07-11 static recipe holds at 1400 °C, giving +13.864 kJ/mol-O₂ standard-state headroom; operation above the crossover is not part of the static recipe and needs future optimizer-owned local thermite/exotherm evidence. Across the executable C6-stage residual sweep, the 1400-1450 °C Al-yield spread is numerical noise for lunar mare low/high-Ti, lunar highland, and Mars basalt, so within-noise yield ties break toward margin headroom and lower heating energy. CI carbonaceous chondrite stays a typed refusal because its staged residual remains below the liquid-fraction workability threshold.
- Ranking at moderate T: Ca > Mg > Al > Ti > Mn > Cr > Fe > Na > K (per the JANAF-4th multiphase refit). The Fe/Na/K corner is unstable across T — the K/Fe crossover sits at ≈ 836 °C, Na/Fe at ≈ 1181 °C, so above those Ts the ladder inverts and the shuttle no longer drives the reduction.
...
A drop of 11 decades in pO₂ (1 atm → ntorr) scales metal activity by ~560× for Na/K, ~3×10⁵× for Fe/Mg/Ca, ~5.6×10⁸× for Cr/Al, and ~10¹¹× for Ti. **Vacuum helps the more-oxidized species most.** Low pO₂ makes the full Ellingham activity ladder accessible at solar-furnace temperatures, but activity alone does not guarantee evaporation: the live pressure is `P_eff = a_M × P_sat`, so the Ca/Al/Ti refractory tail remains below the volatility floor at every furnace-survivable temperature.
...
These are the Sense-2 ladder slopes (theoretical, from stoichiometry). The builtin authoritative vapor-pressure provider applies `pO2_exponent` generically when a row declares it in `data/vapor_pressures.yaml`; the released table uses it for K and CrO₂. The SiO₂ ⇌ SiO + ½ O₂ branch has its own dedicated pO₂ correction, where pO₂ is *the* lever. Metal activities otherwise come from the Ellingham table. VapoRock may report a diagnostic shadow gas-speciation surface, but it does not provide the authoritative `a_M(T, pO₂)` surface consumed by evaporation.
...
pN₂ is the overhead sweep gas. The canonical symbol is N₂; the recommended lineup is Kr + N₂, with CO₂ the natural choice on Mars feedstocks and Ar demoted to a legacy option because its boiling point sits a ~3 K knife-edge from LOX and fouls the downstream cryo cut, where Kr's 119.7 K condenses ~30 K above LOX for a clean first cut (`simulator/transport_constants.py::COLLISION_DIAMETERS_M`, the Kr row's comment; the switch is recorded there as owner-directed). The buffer gas carries two load-bearing duties, not one. The first is transport control: it keeps the Knudsen number well below 0.01 so evolved vapor is swept toward designated condensers rather than crossing the pipe ballistically and landing on whatever cold surface it encounters first. The second is dilution — holding each condensable's partial pressure below its saturation pressure at the wall, which is what makes the hot-wall specification affordable. Both duties are load-bearing; see the paragraph below on why the operating band sits well above the transport floor.

The target band is **5–15 mbar**, typically 8–12 mbar, in a 12 cm typical hot-wall Stage 0 duct (`data/setpoints.yaml` §6). The band keeps the mean-free-path short relative to the pipe diameter, but it is **not** the molecular-flow boundary this document previously claimed it was. Premise: hard-sphere kinetic theory on the simulator's own constants (`simulator/transport_constants.py` — σ_N₂ = 3.798×10⁻¹⁰ m; viscous `Kn < 0.01`, free-molecular `Kn ≥ 10`), N₂ at 1500 °C, `L` = 0.12 m. Algebra: `λ = k_B T / (√2 π σ² P)` and `Kn = λ / L`, so `Kn ∝ 1/P` and the viscous boundary sits at `P = k_B T / (√2 π σ² × 0.01 × L)` = (1.380649×10⁻²³ × 1773.15) / (4.4429 × (3.798×10⁻¹⁰)² × 0.01 × 0.12) = 31.8 Pa ≈ **0.318 mbar**. Unit check: `k_B T` is J = Pa·m³ and `σ² L` is m³, so the quotient is Pa. Sanity check: the same formula at 1 atm, 25 °C gives λ ≈ 63 nm against the tabulated ~68 nm mean free path of air, and the limiting cases land at λ = L near 0.0032 mbar and the free-molecular threshold (`Kn = 10`) near 3×10⁻⁴ mbar. So at the band's 5 mbar low edge `Kn = 6.4×10⁻⁴` — sixteen-fold inside the viscous regime — and even at 1 mbar `Kn = 0.0032`, still viscous with λ 314× smaller than the pipe. The floor scales linearly with T: 0.30 mbar at 1400 °C and 0.35 mbar at 1670 °C, both already recorded with this same derivation in `data/setpoints.yaml` (`continuum_pressure_bounds`).

The band is therefore **engineering margin, ~16× above the viscous boundary — not a flow-regime transition**. The margin is plausibly load-bearing: the buffer gas has a second recorded duty besides transport control, also load-bearing (CLAUDE.md §4) — diluting the condensable vapors so walls stay clean — which argues for pressure well above the transport floor, and capacity, geometry variation, and control margin point the same way. But no derivation fixing 5–15 mbar specifically is recorded in the repo; the reason for the ~16× factor is an acknowledged gap, not a number to reverse-engineer. Above the band the trade runs the other way: heavier dilution cuts each condensable's partial pressure, and with it the thermodynamic driving force at the condenser.
...
The reductant shuttle is one half of a symmetric pair. The other half — **melt-side O₂ injection** (a
bubbler/sparge valve below the melt surface) — is design-accepted (owner, 2026-07-09; ticket t-173, rides the
BH-124 physical-pressure redesign) but **not yet in the model**: today oxygen reaches the melt only through the
headspace ↔ melt-fO₂ surface exchange. Once built, melt fO₂ becomes a fully actuated state variable: the
reductant hierarchy (Na/K → Mg → Ca) drives it *down*, the O₂ valve drives it *up* — both as physical,
flow-capped valve settings the optimizer controls directly. The intended uses are redox-balance-driven burnout
of non-rock ingredients on messy feedstocks (the Stage-0 class: carbon, sulfides, phosphorus compounds,
organics — making that cleanup optimizer-accessible rather than script-only) and general process control.
Oxygen is process-abundant (it is the terminal product), so the flow cap is generous; injected O₂ debits the
terminal O₂ account — the same product-and-reagent doctrine as the alkali shuttle, applied to oxygen. This also
restores fidelity to the mandate's §3 description of Stage-0 ("passing O₂ … through the melt"), which the
current model abstracts into fixed disposition outcomes.
...
**Buffer gas in the viscous-flow regime.** The hard requirement is the regime, not the band: the overhead gas must hold `Kn = λ / L < 0.01` (λ the gas mean-free-path, L the pipe diameter), or evolved vapor stops being swept toward its designated condenser and crosses the duct ballistically to whatever cold surface it encounters first. That floor is ~0.318 mbar at 1500 °C in the 0.12 m duct (derivation under §pN₂ above); the 5–15 mbar operating band holds it with a ~16× engineering margin rather than marking the boundary, and truly ballistic transport (λ = L) needs ~0.003 mbar. What is non-negotiable is the viscous regime itself.
...
The simulator tracks molar wall inventory in `process.wall_deposit` and `process.wall_deposit_segment_*` ledger accounts, then projects those accounts into the per-species `wall_deposit_kg` runner output. This is the fraction of evolved vapor that lands on walls instead of reaching the designated condenser, parameterized by gas-phase conditions (pN₂ / Knudsen regime) and wall temperature. The per-species breakdown exposes which species is the fouling problem at a given operating point.

The operational target is not zero deposited mass but long-term operation without re-sintering. A run that completes once and then requires furnace rebuild before the next batch is not a working recipe. The `wall_deposit_kg` projection exists to make the fouling story legible without running physical hardware.
...
- **Melt Fe redox is only partly grounded.** The Fe vapor-pressure path now uses the Kress & Carmichael 1991 ferric/ferrous split as the live `a_FeO` authority: CALPHAD/Holzheid metal-saturated behavior below roughly IW, Kress91 above roughly IW+1, with a smooth blend and `a_FeO <= 1.0`. The intrinsic fO₂ source that feeds that split is still an ungrounded cleaned-melt composition heuristic / diagnostic input, and the switch is still anchored to pure-FeO IW rather than a self-consistent basaltic metal-saturation point.
- **Metal-phase settling is instrumented, not behavior-gated.** Builtin-authored reduced metal is committed into mol-native `process.metal_phase_bottom_pool` (Fe-family plus contact-scavenged Si) and `process.metal_phase_float_layer` (Al-Si) accounts. When a backend-authored transition controls `process.metal_phase`, that account remains authoritative: the builtin stratifier emits read-only bottom/float projections and does not commit its derived disposition. A first-order `k_mix` contact diagnostic, engine-melt-density buoyancy verdict, alloy wt% projection, and equivalent interface-film thickness are reported. Existing product taps and extraction routing remain unchanged; coherent-layer suppression of evaporation, redox exchange, or gas escape is a separate later gate. The default operating assumption is induction mixing plus aggressive float tapping to keep the interface open, and the diagnostic states when that assumption becomes load-bearing.
- **Finite overhead headspace** (toggle `overhead_headspace.enabled`) defaults ON. When enabled, O₂ is held in `process.overhead_gas`; Poiseuille conductance is an input to the bleed calculation, while cold-train hardware ratings reduce to the authoritative capacity `C`. The bleed provider partitions O₂ among admitted/stored, accumulated, relieved, and held legs. Turbine fields are read-only projections of that ledgered partition, not a separate capacity authority. Molecular-flow conductance and validated hardware control are out of scope.
...
The truth-seeking discipline for this project: when the model disagrees with reference data, the correct response is investigation, not parameter-tuning to force agreement.
