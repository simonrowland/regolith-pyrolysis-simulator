...
- **Metal-phase settling is instrumented, not behavior-gated.** Reduced metal is committed into mol-native `process.metal_phase_bottom_pool` (Fe-family plus contact-scavenged Si) and `process.metal_phase_float_layer` (Al-Si) accounts. A first-order `k_mix` contact diagnostic, engine-melt-density buoyancy verdict, alloy wt% projection, and equivalent interface-film thickness are reported. Existing product taps and extraction routing remain unchanged; coherent-layer suppression of evaporation, redox exchange, or gas escape is a separate later gate. The default operating assumption is induction mixing plus aggressive float tapping to keep the interface open, and the diagnostic states when that assumption becomes load-bearing.
- **Finite overhead headspace** (toggle `overhead_headspace.enabled`) defaults ON. When enabled, evaporation O₂ is held in `process.overhead_gas` and bled through a Poiseuille conductance model; molecular-flow conductance and validated hardware control are out of scope.
