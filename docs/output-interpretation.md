...
- **`metal_projection_drift_kg`** (W8 / M2 closure) — per-species `ledger_kg - projection_kg` when the combined AtomLedger backing mass in the three metal-phase accounts (`process.metal_phase`, `process.metal_phase_bottom_pool`, and `process.metal_phase_float_layer`) plus `process.condensation_train` differs from the sum across `train.stages[*].collected_kg` (UI projection) by more than `1×10⁻⁹ kg`. Empty dict means stage collection is in sync with its ledger backing. Negative values surface a projection-only stale state (UI carries phantom kg with no ledger backing) — that case is rare in production but `union_iteration` (post-push P2 fix) makes it visible rather than silent. Under the runner's strict result contract, any nonempty drift fails the run with `reason="metal_projection_drift"`; positive and negative drift receive the same treatment because either sign is an operator-visible ledger/projection inconsistency. Interactive consumers preserve nonempty drift as a diagnostic only. The global `mass_balance_error_pct` ≤5×10⁻¹² % gate remains hard in every mode.
