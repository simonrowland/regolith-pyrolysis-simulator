...
|---|---|---|---|---|
| Na | runtime ceiling 1.0; physical envelope 0.0-1.0 | Sossi et al. 2019 GCA 260:204 reports a gamma-derived 0.3-1.7 inference at 1400 C, separately adopts alpha_e=1, and states alpha_e<=1 | The former 0.9-1.0 interval is unsupported and withdrawn. Unity is a status-bearing analytical upper bound, not a measurement or residual pin. | analytical ceiling (not pinnable) |
| K | 0.13 (tier 2); recommended band 0.10-0.16 | Fedkin et al. 2006 LPSC 37:#2249 KEMS sealed-chamber intrinsic K `alpha_e~0.13` | Replaces the prior Na open-furnace analogy for the series-resistance intrinsic-alpha model; Sossi open-furnace near-unity behavior remains a regime-mismatch warning, not this runtime pin. | pinned (Yes (pinnable)) |
...
| CrO₂ | 0.9 (envelope 0.48-1.2, 1400-2273.15 K, tier 2, tag `broad_proxy_not_intrinsic`) — carried as proxy evidence, refused at the measured-alpha gate | No accessible condensed-phase CrO₂ HKL α; CrO₂ is a gas-phase redox product. The 0.9 is inherited wholesale from the Cr metal family, not measured for CrO₂ | **FIXED 2026-08-28 (b-314): the tag is now wired into flux admission.** `_load_evaporation_alpha_by_species` emits the row as a provenance-carrying mapping (`alpha_proxy_tag`, tier, source — never a bare number), and the provider-facing `_measured_alpha_control_view` strips it so the presence-based `_alpha_is_unmeasured` gate refuses: CrO₂ enters `missing_alpha` again (SC-67, retained condensed) unless `chemistry_kernel.allow_unmeasured_alpha_fallback` is explicitly set, which takes the marked alpha=1.0 prototype path recorded in `unmeasured_alpha_fallback_species`. The pre-fix state measured earlier the same day: a bare `CrO2 -> 0.9` satisfied the gate and the SC-67 refusal never fired. Same wiring covers every `broad_proxy_not_intrinsic` row (AlO/Al2O/Al2/Al2O2/Al2O3(g)/AlO2/CrO) as a class. | ungrounded proxy, refused at gate (b-314) |
...
| Total non-O2 vapor/capture | **Unexplained residual** | residual | active melt fraction, heat penetration, and oxidation-state target identity | Use the `1.1-2.0x` band as a target-uncertainty envelope consistency check before claiming improved vapor/capture accuracy. |
...
| Per-surface deposition / oxidation state | Surface and sticking bands | model / experiment | Fe on cold Cu/SS near `1`; Na `0.5-1.0`; SiO HOT evaporation uses `0.52*exp(-3685/T_K)` while COLD-WALL condensation below `1000 K` uses Pound 1972 unity with band `[0.016,1.0]`; Mg broad `0.04-1.0`; wall `100-400+ deg C`, condenser `50-200+ deg C` | Surface temperature, view factors, sticking, and re-evaporation remain unresolved; SiO cold-wall sticking is grounded by the high-supersaturation limit, with no direct cold-wall SiO measurement. |
...
| 7 | Alpha | Na `alpha=1.0` analytical ceiling, envelope `[0.0,1.0]` | physical upper bound; not a measurement | `data/vapor_pressures.yaml::metals.Na.evaporation_alpha`; withdrawal guarded by `tests/test_evaporation_alpha_provenance.py` | Medium; Na is minor vapor mass. No mass-spec residual pin remains. |
| 8 | Alpha | K `alpha=0.13`, envelope `[0.10,0.16]`, Fedkin sealed-chamber intrinsic coefficient | literature-pinned | `data/vapor_pressures.yaml::metals.K.evaporation_alpha` | Medium/low; regime-specific intrinsic-alpha pin. |
...
| 43 | Sticking | Runtime default alpha_s now single-sourced from `data/literature/vacuum_pyrolysis_sticking.yaml`: Fe .02, SiO hot-source Arrhenius plus `cold_wall_condensation=1.0`, Mg .20, Na 1.0 are CITED; K/Ca/Cr/Mn/CrO2 are `UNCERTIFIED` high-capture defaults | mixed: literature-pinned plus UNCERTIFIED | `data/literature/vacuum_pyrolysis_sticking.yaml`, `simulator/condensation.py`, `data/materials.yaml` refs | Very high for named wall/deposit capture; SiO cold-wall unity is grounded by Pound 1972 but still lacks a direct cold-wall SiO measurement; chem-lens review still required for surface-transfer applicability. |
| 44 | Sticking | Unknown species fallback `alpha_s=0.8` is `UNCERTIFIED`, status-bearing, and sourced from `unknown_species_default` in the sticking sidecar | UNCERTIFIED numerical default | `data/literature/vacuum_pyrolysis_sticking.yaml::unknown_species_default` | Medium if unlisted species appears. |
| 45 | Wall materials | `data/wall_materials.yaml` stickiness rows still reference `data/materials.yaml::default_alpha_s_by_species`, but those defaults are refs into the sticking sidecar; material-specific fail-closed overrides remain status-bearing | mixed: sidecar-backed plus material overrides | `data/wall_materials.yaml`, `data/materials.yaml`, `data/literature/vacuum_pyrolysis_sticking.yaml` | High; direct material-surface measurements remain open for coating gate arming. |
