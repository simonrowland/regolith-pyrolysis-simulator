...
| SiO (from SiO2 melt) | HOT evaporation `0.52*exp(-3685/T_K)`; valid range `1000-1800 K`; uncertainty envelope `[0.003,0.067]`; COLD-WALL condensation below the floor uses `1.0` with band `[0.016,1.0]` | Wetzel & Gail 2013 A&A 553 A92 DOI 10.1051/0004-6361/201220803 for source evaporation; Pound 1972 JPCRD 1:135 DOI 10.1063/1.3253096 for high-supersaturation cold-wall unity condensation | Runtime authority is temperature-dependent alpha_s(T) for HOT source evaporation only. The prior `0.04` mid-range value remains provenance only. Cold walls below 1000 K use the grounded Pound unity condensation rule; `alpha_e != alpha_c` off-equilibrium, and the no-direct-cold-wall-SiO-measurement gap remains explicit. | pinned T-form + cited cold-wall gate (CITED) |
...
| Mn, CrO₂ | tier 3 (no value); recommended band N/A (fail-loud unless fallback) | Mn awaits a second owner-requested literature sweep; no accessible condensed-phase CrO₂ HKL α, and CrO₂ is a gas-phase redox product | SC-67 refusal remains; explicit fallback only. | assumption (Assumption-only) |
...
| 11 | Alpha | SiO HOT evaporation `alpha=0.52*exp(-3685/T_K)`, valid `1000-1800 K`, envelope `[0.003,0.067]`; COLD-WALL condensation below the floor uses Pound 1972 unity, band `[0.016,1.0]` | literature-pinned | `data/vapor_pressures.yaml::oxide_vapors.SiO.evaporation_alpha`; `data/literature/vacuum_pyrolysis_sticking.yaml::species.SiO.cold_wall_condensation`; Wetzel & Gail 2013 DOI 10.1051/0004-6361/201220803; Pound 1972 DOI 10.1063/1.3253096 | Very high; SiO dominates non-O2 vapor, and cold-wall direct-measurement coverage remains a gap. |
...
| 16 | Alpha | Cr `alpha=0.9`, envelope `[0.8,1.0]`, tier 1 direct solid-Cr measurement; Mn remains `fail_loud_missing_alpha` tier 3 | mixed: literature-pinned plus ungrounded | `data/vapor_pressures.yaml::metals.Cr.evaporation_alpha`; `data/vapor_pressures.yaml::metals.Mn.evaporation_alpha_policy`; Pound 1972 DOI 10.1063/1.3253096 | Cr is valid only for clean pure-solid free evaporation over 1318–1563 K; Mn awaits the second owner-requested literature sweep. Melt activity remains in `P_sat`. |
