...
This document covers the interactive Flask + Socket.IO plane only. The separate CLI optimizer / batch eval plane — warm worker processes, the `EvalSpec` cache layers, and Studio precompute — is documented in [`architecture-eval-runtime-2026-06.md`](architecture-eval-runtime-2026-06.md). The two planes share the chemistry kernel and the mol-native ledger but not the session model: batch evals build a fresh `PyrolysisSimulator` and `AtomLedger` per task inside a pool worker, and `web/events.py` never imports `simulator/optimize/worker_runtime.py`.
