...
- For the optional engines install on macOS arm64: Xcode command-line tools (`xcode-select --install`) and [Homebrew](https://brew.sh) — the installer uses `brew` to fetch `nlopt`, `open-mpi`, and `gsl` before compiling MAGEMin and ThermoEngine from source

No external thermodynamic software is required to run the simulator. Without engines installed, the builtin Ellingham/Antoine fallback path is used; results are comparative estimates, not validated melt chemistry.
...
### Optional engines (macOS arm64)

```bash
python3 install-engines.py
```

This clones and installs PetThermoTools, Thermobar, PySulfSat, and VapoRock as editable siblings of the repo, then compiles MAGEMin and ThermoEngine from source. The alphaMELTS binary is not bundled; see `engines/alphamelts/` for placement notes. The engines installer is idempotent: existing builds are reused.
...
Without engines, the simulator uses the builtin Ellingham/Antoine model for vapor pressure and equilibrium estimates. The `docs/melt-backends.md` backend-selection log line confirms which path is active on each launch.
