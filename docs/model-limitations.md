...
- **Wall re-evaporation uses a per-species reactivity class.** Physisorbing species use the reversible pure-species `P_sat(T_wall)` backstop, so a sufficiently hot wall rebounds the deposit. SiO is classified as reactive because wall capture disproportionates it into lower-volatility products; its re-evaporation backstop is therefore the effective product `P_sat ~= 0`, not SiO's own Antoine curve. Na, K, Ca, Mn, Cr, Al, and Ti are currently modeled as reversible physisorbers. Fe and Mg are also modeled as physisorbing by default, with a known reactive-tail approximation: in real refractory liners they may alloy with or reduce some oxide surfaces, but that secondary sink is not modeled until a separate wall-chemistry grounding chunk. CrO2 is kept on the reversible path in the current SiO-only fix; Cr oxide wall-product irreversibility remains a separate decision.
...
- **SiO sticking/evaporation α is temperature-dependent and range-flagged.** The runtime uses the Wetzel & Gail 2013 Arrhenius compilation `alpha_s_SiO(T)=0.52*exp(-3685/T)` for both source evaporation and wall/condenser sticking by microscopic reversibility. The grounded range is about 1000-1800 K with an uncertainty envelope of 0.003-0.067. Colder Stage-3/below-wall evaluations still use the Arrhenius value rather than clamping, but diagnostics record the valid-range extrapolation flag; those cold-wall fouling rates carry extra uncertainty until direct low-temperature deposition data exists.
