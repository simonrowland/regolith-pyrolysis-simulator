...
- **MRE decomposition-voltage ladder:** NiO rung (0.39 V) is raw-thermo-grounded (`ΔfG°(NiO, 1873 K) ≈ -76 kJ/mol`, Hemingway 1990 + NEA); legacy rungs (FeO 0.6 etc.) are empirically compressed without per-rung provenance and on a different absolute scale. Only the relative ordering is anchored. Whole-ladder re-anchoring to one raw-thermo basis is tracked (#32). Voltages are standard-state; runtime applies the Nernst melt-activity + pO2 correction.
- CoO is intentionally excluded from the MRE ladder: CoO E_decomp is about 0.49 V (Holmes 1986), above the NiO 0.39 V floor, and modeled cobalt feedstock is trace siderophile/native Fe-Ni-Co metal rather than a CoO MRE target. The ladder remains mixed-scale (raw-thermo NiO 0.39 V plus legacy FeO 0.6 V and descendants) pending the chunk-B reanchor.
