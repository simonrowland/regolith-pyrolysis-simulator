...
- The builtin K/Na FeO-shuttle reactions (`2K + FeO -> K2O + Fe`, `2Na + FeO -> Na2O + Fe`) are temperature-independent. The V1c JANAF refit puts the approximate FeO crossovers at K/Fe ~832 C and Na/Fe ~1173 C, while the current `C2A_staged` recipe still cools below ~1200 C before C3_K. Recipe retune is tracked separately; the engine does not yet enforce this temperature gate. Temperature-gated ferric/ferrous redox fidelity is future engine work.
