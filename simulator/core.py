...
class PyrolysisSimulator(EquilibriumMixin, EvaporationMixin, ExtractionMixin):
    ...
    def _observe_reagent_provenance_transition(
        ...
        amalgamated_elements_by_account: Dict[str, set[str]] = {}
        amalgamated_credit_accounts = {
            SPENT_REDUCTANT_RESIDUE_ACCOUNT,
            'process.overhead_gas',
            'process.condensation_train',
        }
        for lot in transition.credits:
            if lot.account not in amalgamated_credit_accounts:
                continue
            for species in lot.species_kg:
                ...
                amalgamated_elements_by_account.setdefault(
                    lot.account,
                    set(),
                ).update(formula.elements)
        for account, elements in amalgamated_elements_by_account.items():
            self.atom_ledger.mark_amalgamated_pool(
                account,
                elements,
            )
    ...
    def _c3_alkali_accounting_diagnostic(self) -> dict:
        termination = dict(
            getattr(self.campaign_mgr, 'last_c3_termination', {}) or {}
        )
        significance_limits = dict(
            termination.get(
                'significant_melt_remaining_limit_kg_by_species',
                {},
            ) or {}
        )
        origin_atoms = self.atom_ledger.origin_atom_moles_by_account()
        credit_drawn = dict(
            getattr(
                self,
                '_c3_alkali_credit_drawn_kg_by_species',
                {},
            ) or {}
        )
        credit_outstanding = (
        ...
        for species, oxide in (('Na', 'Na2O'), ('K', 'K2O')):
            element_mass_kg_per_mol = resolve_species_formula(
                species,
                self.species_formula_registry,
            ).molar_mass_kg_per_mol()
            reagent_origin_kg_by_account = {}
            for account, element_origins in origin_atoms.items():
                reagent_mol = float(
                    element_origins.get(species, {}).get('reagent', 0.0)
                )
                reagent_kg = reagent_mol * element_mass_kg_per_mol
                if reagent_kg > 0.0:
                    reagent_origin_kg_by_account[str(account)] = reagent_kg

            melt_bound_reagent_origin_kg_by_account = {
                account: reagent_origin_kg_by_account.get(account, 0.0)
                for account in (
                    'process.cleaned_melt',
                    SPENT_REDUCTANT_RESIDUE_ACCOUNT,
                )
                if reagent_origin_kg_by_account.get(account, 0.0) > 0.0
            }
            melt_bound_element_kg = sum(
                melt_bound_reagent_origin_kg_by_account.values()
            )
            total_positive_kg = sum(reagent_origin_kg_by_account.values())
            significant_limit_kg = max(
                0.0,
                float(significance_limits.get(species, 0.0) or 0.0),
            )
            significant_melt_bound = (
                melt_bound_element_kg > significant_limit_kg
            )
            ...
            recoverable_credit_kg_by_account = {
                account: kg
                for account, kg in reagent_origin_kg_by_account.items()
                if account in recoverable_accounts
            }
            wall_deposit_loss_kg_by_account = {
                account: kg
                for account, kg in reagent_origin_kg_by_account.items()
                if account in wall_accounts
            }
            irrecoverable_vent_loss_kg_by_account = {
                account: kg
                for account, kg in reagent_origin_kg_by_account.items()
                if account in irrecoverable_vent_accounts
            }
            recoverable_credit_kg = sum(
                recoverable_credit_kg_by_account.values()
            )
            wall_deposit_loss_kg = sum(
                wall_deposit_loss_kg_by_account.values()
            )
            irrecoverable_vent_loss_kg = sum(
                irrecoverable_vent_loss_kg_by_account.values()
            )
            irrecoverable_loss_kg = (
                wall_deposit_loss_kg + irrecoverable_vent_loss_kg
            )
            non_disposition_accounts = {
                'process.cleaned_melt',
                SPENT_REDUCTANT_RESIDUE_ACCOUNT,
                'process.reagent_inventory',
                f'reservoir.reagent.{species}',
            }
            classified_accounts = (
                recoverable_accounts
                | wall_accounts
            ...
            unclassified_disposition_kg_by_account = {
                account: kg
                for account, kg in reagent_origin_kg_by_account.items()
                if account not in classified_accounts
            }
            unclassified_disposition_kg = sum(
                unclassified_disposition_kg_by_account.values()
            )
            disposition_loss_detected = irrecoverable_loss_kg > 0.0
            significant_disposition_loss = (
                irrecoverable_loss_kg > significant_limit_kg
            )
            disposition_unclassified = unclassified_disposition_kg > 0.0
            by_species[species] = {
                'credit_line': {
                    'requested_inventory_kg': (
                        self._c3_alkali_requested_dose_kg(species)
                    ),
                    'gross_drawn_kg': max(
                        0.0,
                        float(credit_drawn.get(species, 0.0) or 0.0),
                    ),
                    'net_outstanding_kg': max(
                        0.0,
                        float(credit_outstanding.get(species, 0.0) or 0.0),
                    ),
                    'available_reagent_inventory_kg': (
                        reagent_origin_kg_by_account.get(
                            'process.reagent_inventory',
                            0.0,
                        )
                    ),
                    'unused_source_reservoir_kg': (
                        reagent_origin_kg_by_account.get(
                            f'reservoir.reagent.{species}',
                            0.0,
                        )
                    ),
                    'reagent_origin_total_kg': total_positive_kg,
                },
                'melt_clearance': {
                    'status': (
                        'incomplete'
                        if significant_melt_bound
                        else 'clear'
                    ),
                    'reagent_origin_remaining_in_melt_kg': (
                        melt_bound_element_kg
                    ),
                    'reagent_origin_outside_melt_kg': max(
                        0.0,
                        total_positive_kg - melt_bound_element_kg,
                    ),
                    'reagent_origin_remaining_in_melt_kg_by_account': (
                        melt_bound_reagent_origin_kg_by_account
                    ),
                    'oxide_equivalent_remaining_in_melt_kg': (
                        melt_bound_element_kg / STOICH_RATIOS[oxide][0]
                    ),
                    'significant_remaining_limit_kg': significant_limit_kg,
                    'significant_remaining': significant_melt_bound,
                },
                'disposition': {
                    'status': (
                        'unclassified'
                        if disposition_unclassified
                        else 'irrecoverable_loss_detected'
                        if disposition_loss_detected
                        else 'no_irrecoverable_loss_detected'
                    ),
                    'recoverable_credit_kg': recoverable_credit_kg,
                    'recoverable_credit_kg_by_account': (
                        recoverable_credit_kg_by_account
                    ),
                    'irrecoverable_loss_kg': irrecoverable_loss_kg,
                    'wall_deposit_loss_kg': wall_deposit_loss_kg,
                    'wall_deposit_loss_kg_by_account': (
                        wall_deposit_loss_kg_by_account
                    ),
                    ...
                    'unclassified_kg': unclassified_disposition_kg,
                    'unclassified_kg_by_account': (
                        unclassified_disposition_kg_by_account
                    ),
                    'irrecoverable_loss_detected': disposition_loss_detected,
                    'unclassified': disposition_unclassified,
                    'significant_loss_limit_kg': significant_limit_kg,
                    'significant_irrecoverable_loss': (
                        significant_disposition_loss
                    ),
                    'wall_loss_provisional_pending_t475': True,
                    'wall_loss_caveat': (
                        'Wall-loss magnitude is provisional pending t-475: '
                        'wall local pressure currently uses pure-component '
                        'P_sat(T_melt) instead of species partial pressure.'
                    ),
                },
                'oxide_species': oxide,
            }
        melt_clearance_incomplete = any(
            bool(row['melt_clearance']['significant_remaining'])
            for row in by_species.values()
        )
        disposition_loss_detected = any(
            bool(
                row['disposition']['irrecoverable_loss_detected']
            )
            for row in by_species.values()
        )
        disposition_unclassified = any(
            bool(row['disposition']['unclassified'])
            for row in by_species.values()
        )
        return {
            'melt_clearance_status': (
                'incomplete'
                if melt_clearance_incomplete
                else 'clear'
            ),
            'melt_clearance_incomplete': melt_clearance_incomplete,
            'disposition_status': (
                'unclassified'
                if disposition_unclassified
                else 'irrecoverable_loss_detected'
            ...
            'disposition_loss_detected': disposition_loss_detected,
            'disposition_unclassified': disposition_unclassified,
    ...
    def _capture_campaign_summary(self, campaign_name: str) -> dict:
        ...
        if campaign_name == 'C7_CA_ALUMINOTHERMIC':
            ...
            accounting = self._c3_alkali_accounting_diagnostic()
            if accounting['melt_clearance_incomplete']:
                termination['melt_clearance_status'] = 'incomplete'
                termination['melt_clearance_incomplete'] = True
                termination['severity'] = 'warning'
            if accounting['disposition_loss_detected']:
                termination['disposition_status'] = (
                    'irrecoverable_loss_detected'
                )
                termination['disposition_loss_detected'] = True
                termination['severity'] = 'warning'
            if accounting['disposition_unclassified']:
                termination['disposition_status'] = 'unclassified'
                termination['disposition_unclassified'] = True
                termination['severity'] = 'warning'
            summary['c3_termination'] = termination
            summary['c3_alkali_accounting'] = accounting
        ...
        try:
            return self.campaign_mgr.check_endpoint(
                ...
                transport_state={
                    ...
                    'evaporation_flux_diagnostic': dict(
                        getattr(
                            self,
