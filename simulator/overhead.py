...
class OverheadGasModel:
    ...
    def update(self, evap_flux: EvaporationFlux,
        ...
        # ── Overhead pressure ───────────────────────────────────────
        # P ≈ (evap_rate / conductance) × characteristic pressure
        if conductance > 0:
            gas.pressure_mbar = (total_evap_kg_hr / 3600.0) / conductance * 10.0
        else:
            gas.pressure_mbar = melt.p_total_mbar

        # Add controlled atmosphere pressure
        gas.pressure_mbar = max(gas.pressure_mbar, melt.p_total_mbar)

        # ── Partial pressures (proportional to evaporation rates) ──
