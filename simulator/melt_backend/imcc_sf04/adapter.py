...
``ImccSf04Backend`` / ``ImccSf04ExtBackend`` are the thin MeltBackend
surface used by ``resolve_backend``. They are shadow / diagnostic only.
Promotion into ``REAL_MELT_BACKEND_NAMES`` and active recipe eligibility
is a separate owner-gated change after the battery result (t-890).
"""
...
_PUBLISHED_DATAPACK_PATH = (
    _REPO_ROOT / "data" / "melt_activity" / "imcc" / "imcc-sf04-v1.0.2.json"
)
_EXT_DATAPACK_PATH = (
    _REPO_ROOT / "data" / "melt_activity" / "imcc" / "imcc-sf04-ext-v4.json"
)
