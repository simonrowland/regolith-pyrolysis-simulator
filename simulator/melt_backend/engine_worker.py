"""Reusable persistent-process isolation for native engine adapters.
...
class WarmEngineWorker:
    ...
    def start(self, *, timeout_s: Optional[float] = None) -> Any:
        ...
        try:
            ...
            if not parent.poll(startup_timeout):
                raise TimeoutError(
                    f'{self.name} initialization exceeded hard timeout of '
    ...
    def call(self, request: Any, *, timeout_s: Optional[float] = None) -> Any:
        ...
        if remaining <= 0.0:
            ...
            raise TimeoutError(
                f'{self.name} exceeded hard timeout of {timeout:g}s'
            )
        try:
