...
class OverheadGas:
    ...
    turbine_flow_kg_hr: float = 0.0  # Read-only mirror of OVERHEAD_BLEED o2_stored_mol.
    # Mass flow rate through turbine (sets pO₂)

    turbine_flow_mol_hr: float = 0.0  # Read-only mirror of OVERHEAD_BLEED o2_stored_mol.
    ...
    O2_vented_kg_hr: float = 0.0  # Read-only mirror of OVERHEAD_BLEED melt_o2_vented_mol.
    # Melt-offgas O₂ vented to lunar vacuum this hour (cold-train relief);
    # excludes the external/bubbler O₂ stream, which is ledgered separately.

    O2_vented_mol_hr: float = 0.0  # Read-only mirror of OVERHEAD_BLEED melt_o2_vented_mol.
    # Melt-offgas O₂ molar flow vented this hour (excludes external O₂)
