"""NIST WebBook Shomate polynomial evaluator (third vapour-rail thermo family).

Typed family for catalog rows that declare ``evaluator_family: shomate`` (or
``evaluator: shomate``). Coefficients follow the NIST Chemistry WebBook
convention (Chase 1998 / SRD 69) — **not** a re-fit of spreadsheet rows.
...
Segment contracts match the NASA family: contiguous non-overlapping cover,
loud gap/overlap, missing standard-state convention, and domain failures.
Phase-transition **locations** are never minted here — Ellingham remains the
single home for physical breakpoints; Shomate supplies energetics only.
"""
...
class ShomateSegment:
    ...
    def evaluate(self, T_K: float) -> ThermoState:
        ...
        T = float(T_K)
        if T <= 0.0:
            raise ShomateDomainError(f"Shomate requires T > 0 K; got {T}")
        t = T / 1000.0
        t2 = t * t
        t3 = t2 * t
        t4 = t3 * t
...
class ShomatePolynomial:
    ...
    def formation_enthalpy_J_per_mol(self) -> float:
        ...
        which WebBook tables carry as ΔfH°298 for the species. Multi-segment
        records must agree on ``H`` to within 1e-6 kJ/mol.
        """
        ...
        if any(abs(h - h_kJ[0]) > 1.0e-6 for h in h_kJ[1:]):
            raise ShomateConventionError(
...
def coefficients_from_mapping(raw: object) -> tuple[float, ...]:
    ...
    if isinstance(raw, Mapping):
        try:
            return tuple(float(raw[k]) for k in _SHOMATE_KEYS)  # type: ignore[index]
