(function registerMrePanel(root) {
  ...
  function renderDirectReadouts() {
    // Target emits summary.mre_voltage_V and summary.mre_current_A; these
    // cards are unbound in this panel, not absent from the artifact.
    const unbound = (label) =>
      `<div><span>${esc(label)}</span><b>Not bound</b><small>Not bound in this panel</small></div>`;
    const pendingProducer = (label) =>
      `<div><span>${esc(label)}</span><b>Pending producer</b><small>Not emitted in the report artifact</small></div>`;
    return `<div class="sec-p6-direct" aria-label="Direct MRE readouts">` +
      `${unbound("Applied cell voltage · V")}${unbound("Cell current · A")}${pendingProducer("Metals production rate · kg/h")}</div>`;
  }
