...
function runCard(run) {
  ...
  return `<article class="card run-card">
    <div class="run-card-head">
      ...
      <button class="star-button${isStarred ? " active" : ""}" type="button" data-star="${esc(runId)}" aria-pressed="${isStarred}" aria-label="${esc(`${isStarred ? "Remove" : "Add"} ${displayName} ${isStarred ? "from" : "to"} favorites`)}"${isStarPending ? " aria-disabled=\"true\" aria-busy=\"true\"" : ""}>${isStarred ? "★" : "☆"}</button>
    </div>
...
function focusedControlTarget() {
  ...
  if (active?.dataset?.star !== undefined) {
    return {
      attribute: "star",
      value: active.dataset.star,
      fallback: { attribute: "folder", value: activeFolder }
    };
  }
