...
_CAMPAIGN_SUMMARY_HARNESS = (
    _REPO_ROOT
    / "tests"
    / "fixtures"
    / "web_render"
    / "render_campaign_summary_dom.mjs"
)
    ...
    (
        {
            ...
            "duration_h": 35,
            "mass_lost_kg": 12.0,
            "start_mass_kg": 1038.0,
            "end_mass_kg": 1026.0,
            "energy_electrical_plus_evaporation_kWh": 100.0,
            "energy_scope": "partial",
            ...
            "c3_alkali_accounting": {
                "melt_clearance_status": "clear",
                "melt_clearance_incomplete": False,
                "disposition_status": "no_irrecoverable_loss_detected",
        ...
        },
        {
            ...
            "c3_alkali_accounting": {
                "melt_clearance_status": "incomplete",
...
def test_c3_incomplete_summary_renders_open_operator_warning(payload):
    completed = subprocess.run(
        ["node", str(_CAMPAIGN_SUMMARY_HARNESS)],
        input=json.dumps(
            {
                "payload": payload,
                "script_path": str(_SIMULATOR_CONTROLS_JS),
            }
        ),
        text=True,
    ...
    assert "campaign-summary-warning" in rendered["className"]
    assert "WARNING — Incomplete" in rendered["text"]
    assert "C3 burn-out status" in rendered["text"]
    assert "C3 melt-clearance status" in rendered["text"]
    assert "C3 disposition status" in rendered["text"]
    assert "Operator warning" in rendered["text"]


def test_c3_zero_dose_summary_renders_not_applicable_not_complete():
    payload = {
        "campaign": "C3_NA",
        "duration_h": 1,
        "mass_lost_kg": 0.0,
        "start_mass_kg": 1000.0,
        "end_mass_kg": 1000.0,
        ...
        "c3_alkali_accounting": {
            ...
            "disposition_status": "no_irrecoverable_loss_detected",
            "disposition_loss_detected": False,
            "disposition_unclassified": False,
        },
    }
    ...
    assert "C3_NA Not applicable" in rendered["text"]
    assert "C3_NA Complete" not in rendered["text"]
    assert "statusnot_applicable" in rendered["text"]
