...
def _direct_card(rendered: str, label: str) -> tuple[str, str]:
    match = re.search(
        rf"<div><span>{re.escape(label)}</span><b>(.*?)</b><small>(.*?)</small></div>",
        rendered,
        re.DOTALL,
    )
    assert match is not None, f"missing direct readout card: {label}"
...
def test_p6_renders_emitted_ladder_yield_o2_and_pending_direct_readouts() -> None:
    ...
    assert _direct_card(direct_region, "Applied cell voltage · V") == (
        "Not bound",
        "Not bound in this panel",
    )
    assert _direct_card(direct_region, "Cell current · A") == (
        "Not bound",
        "Not bound in this panel",
    )
    assert _direct_card(direct_region, "Metals production rate · kg/h") == (
        "Pending producer",
        "Not emitted in the report artifact",
    )
...
def test_p6_absent_subtrees_have_separate_pending_states_without_zeroes() -> None:
    ...
    assert html.count("Pending producer") == 1
    assert html.count("Not bound") >= 2
...
def test_p6_render_handles_null_rows_summaries_and_malformed_terminal_account() -> None:
    ...
    assert null_html.count("Pending producer") == 1
    assert null_html.count("Not bound") >= 2
...
def test_p6_voltage_current_cards_stay_unbound_when_artifact_emits_them() -> None:
    """Target emits mre_voltage_V / mre_current_A; this panel does not bind them.

    Mutation: restoring "Not emitted in the report artifact" on those cards
    would fail even when the artifact carries the required runner keys.
    """
    html = _render(
        _artifact(
            {
    ...
    direct_region = _between(html, '<div class="sec-p6-direct"', "</section>")
    voltage_badge, voltage_caption = _direct_card(
        direct_region, "Applied cell voltage · V"
    )
    current_badge, current_caption = _direct_card(direct_region, "Cell current · A")
    rate_badge, rate_caption = _direct_card(
        direct_region, "Metals production rate · kg/h"
    )

    assert voltage_badge == "Not bound"
    assert current_badge == "Not bound"
    assert voltage_caption == "Not bound in this panel"
    assert current_caption == "Not bound in this panel"
    assert rate_badge == "Pending producer"
    assert rate_caption == "Not emitted in the report artifact"
    assert "2.5" not in direct_region
    assert "12" not in direct_region
    assert html.count("Pending producer") == 1
    assert "Not emitted in the report artifact" not in voltage_caption
    assert "Not emitted in the report artifact" not in current_caption
