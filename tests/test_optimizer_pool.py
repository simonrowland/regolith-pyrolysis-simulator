...
#           -> eval[1] recovers on the replacement worker. Fail-loud edge.
...
def test_process_pool_timeout_reaps_normal_subprocess_child(
    ...
    # eval[0] hangs and must be killed; eval[1] must then recover on the replacement
    # worker in the single-slot pool; and an UNREAPED child must still write its survivor file
