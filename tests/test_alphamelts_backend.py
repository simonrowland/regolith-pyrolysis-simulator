...
def test_alphamelts_python_liquidus_finder_requires_findliq_call():
    """python_api must call findLiq. Skipping the call leaves this test red.

    The reported liquidus is still the frac-M bisection. findLiq is the
    cross-check: a result that disagrees with bisection has to warn.
    """
    ...
    class FakeFinderBackend(AlphaMELTSBackend):
        def __init__(self):
            super().__init__()
            self._mode = 'python_api'
            self.findliq_calls = 0

        def _find_petthermotools_liquidus_C(self, comp_wt, *, pressure_bar, seed_T_C):
            self.findliq_calls += 1
            return 1200.0, ()
    ...
    assert backend.findliq_calls == 1
    assert result.status == 'ok'
    assert result.liquidus_T_C == pytest.approx(1300.0, abs=1.0)
    assert any('findLiq differs' in warning for warning in result.warnings)


def test_alphamelts_python_findliq_none_is_unavailable_without_bisection():
    class FakeFinderBackend(AlphaMELTSBackend):
        def __init__(self):
            super().__init__()
            self._mode = 'python_api'
        ...
        def _find_petthermotools_liquidus_C(self, comp_wt, *, pressure_bar, seed_T_C):
            self.findliq_calls += 1
            return None, ('PetThermoTools findLiq did not return a temperature',)

        def equilibrate(self, temperature_C, **kwargs):
            raise AssertionError('bisection must not run when findLiq returns None')
    ...
    assert any('did not return a temperature' in warning for warning in result.warnings)
